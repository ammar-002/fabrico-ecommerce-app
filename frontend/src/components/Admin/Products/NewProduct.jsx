  ...
  const submitHandler = async () => {
    try {
      ...
      const res = await axios.post(
        `${PRODUCTS_API_END_POINT}/add-product`,
        formData,
        {
          headers: {
      ...
      <div className="flex">
        ...
        <div className="min-h-screen w-full bg-gray-100 flex justify-center py-3">
          <div className="w-full max-w-5xl bg-white rounded-xl shadow-md p-7">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files[0];
                  if (file) {
                    setProduct({ ...product, secondImage: file, secondImagePreview: URL.createObjectURL(file) });
                  }
                }}
                className="w-full p-2 border rounded-lg bg-gray-50 outline-none focus:ring-2 focus:ring-blue-600"
              />
              ...
              <div className="flex flex-wrap gap-2">
                {["XS", "S", "M", "L", "XL", "XXL"].map((sz) => (
                  <button
                    ...
                    className={`px-4 py-2 border rounded-lg text-sm  ${product.size.includes(sz)
            ...
            {/* Button */}
            <button
              disabled={isLoading || !product.title || !product.description || !product.image || !product.category || !product.price || !product.stock}
